...
const getMdFiles = (paths) => {
    ...
    const pathName = path.basename(paths)                           // retorna la última porción del path (para obtener el nombre)
    ...
    if (infoPath.isDirectory() && pathName !== 'node_modules') {    // método que da true && el basename diferente a la carpeta node_modules
        const dirElem = fs.readdirSync(paths);                      //Para leer el directorio, muestra sus elementos en un array
    ...
    //console.log(getMdFiles('D:/Carpeta'))
